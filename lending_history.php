...
<link rel="stylesheet" type="text/css" href="table/css/main2.css">
		...
		<h2 class="mb-4">      &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Owner Lending History  </h2>
...
{
echo "<option value=\" WHERE owner.SiNo = ".$row['SiNo']."\">".$row['Name']."</option>";
...
<input type="date" id="dates" name="dates" maxlength="50" size="30"    value="1990-01-01">
...
window.open("lending_history.php?id=".concat(result).concat("&dates=").concat(d),"_self");
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
}
</script>
<button type="button"  onclick="myFunction()">Click to Search</button>
					...
					<table>
						<thead>
							<tr class="table100-head">
								...
								<th class="column3">Physical Location</th>
								<th class="column3">Checked-in/Checked-out</th>
								<th class="column3">Date of Entry</th>
								<th class="column3">Entry by</th>
						...
						<tbody>
						<?php 
						if ( isset($_GET['id'])&& !empty($_GET['id'])){$var = $_GET['id'];}
						else {$var="";}
						if ( isset($_GET['dates'])&& !empty($_GET['dates']) ){$var2 = " and Entry_Date > '".$_GET['dates']."'";}
						else {$var2="";}
$con=mysqli_connect("localhost","root","","aftc_mess");
// Check connection
if (mysqli_connect_errno())
...
{
	$result = mysqli_query($con,"SELECT assets.Item, assets.ID, assets.Description,assets.Physical_location,asset_history.outin, asset_history.Entry_Date,asset_history.Quantity ,asset_history.Entry_by,owner.Name FROM asset_history LEFT JOIN owner ON asset_history.Checked_Out_to=owner.SiNo LEFT JOIN assets ON asset_history.Asset_ID=assets.ID Where owner.SiNo= ".$row1['SiNo'].$var2);
...
if ( $row['outin'] ==0){$var1 = "Checked-in";}
						else {$var1="Checked-out";}
echo "<td>" . $var1 . "</td>";
