...
{
mysqli_query($con,"Replace INTO `owner`(`SiNo`,`Name`, `Unit`, `Address`, `Phone_number`) VALUES ('".$_GET['ids']."','".$_GET['name']."','".$_GET['unit']."','".$_GET['address']."','".$_GET['phno']."')");

	echo "<script>  window.open('owner_contact.php?itemid=".$_GET['ids']."','_self'); </script>" ;
}

if (isset($_GET['id'])&& !empty($_GET['id']) and $_GET['id'])
{ $row3= mysqli_fetch_array(mysqli_query($con,"SELECT * from owner Where SiNo = ".$_GET['id']));

echo "
<form action=\"owner_add_edit.php\" method=\"get\">
<table width=\"200px\">
</tr>
<br>
<br>
...
<td>
<input type=\"text\" name=\"unit\" maxlength=\"50\" size=\"30\" value=".$row3['Unit'].">
</td>
</tr>
<tr>
<td>
