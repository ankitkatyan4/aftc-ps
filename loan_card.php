...
<link rel="stylesheet" type="text/css" href="table/css/main2.css">
		...
		<h2 class="mb-4">      &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Loan Card  </h2>
<p>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; As of Date/Time: <span id="datetime"></span></p><script>var dt = new Date();
...
{
echo "<option value=\" WHERE loanee_staus.loanee_si = ".$row['SiNo']."\">".$row['Name']."</option>";
...
window.open("loan_card.php?id=".concat(result),"_self");
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
}
</script>
<button type="button"  onclick="myFunction()">Submit</button></div>
					...
					<table>
						<thead>
							<tr class="table100-head">
								...
								<th class="column3">Signature of Store IC</th>
								<th class="column3">Signature of Loanee</th>
							</tr>
						</thead>
						<tbody>
						<?php 
...
$result1 = mysqli_query($con,"SELECT DISTINCT loanee_staus.loanee_si from loanee_staus ".$var." order by loanee_staus.loanee_si");
...
{
$result = mysqli_query($con,"SELECT assets.ID,assets.Item, assets.Description,assets.Physical_location,owner.Name, loanee_staus.loanee_si, loanee_staus.quantity FROM loanee_staus LEFT JOIN owner ON loanee_staus.loanee_si=owner.SiNo LEFT JOIN assets ON loanee_staus.item_si=assets.ID WHERE loanee_staus.loanee_si=".$row1['loanee_si']);
