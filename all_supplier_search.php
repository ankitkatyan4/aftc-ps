...
<link rel="stylesheet" type="text/css" href="table/css/main2.css">
		...
		<h2 class="mb-4">      &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Select Supplier To Edit</h2>
		<div class="wrap-table100">
<form action="all_supplier_search.php" method="get">
		<table>
...
$result = mysqli_query($con,"SELECT DISTINCT City FROM supplier");
...
<input type="Submit" value="Search"></tr></form>
...
function myFunction() {
        ...
        var chks = document.querySelector('input[name=check_list]:checked').value
//		result.localeCompare("Choose Loanee")!=0

if(chks)
{window.open("supplier_add_edit.php?id=".concat(chks),"_self");}
 
else
{window.alert("Please select an Item");}
}
</script>
<button type="button"  onclick="myFunction()">Edit</button></div>
				<div class="table100">
					<table>
						<thead>
						...
						$query='';
						if ( isset($_GET['city']) && !empty($_GET['city']) && strcmp($_GET['city'],"All")!=0)
						{
						$query=" Where City LIKE '".$_GET['city']."'  ";
						}
$con=mysqli_connect("localhost","root","","aftc_mess");
// Check connection
if (mysqli_connect_errno())
...
$result = mysqli_query($con,"SELECT * FROM supplier ".$query);


while($row = mysqli_fetch_array($result))
{
echo "<tr>";
echo "<td><input type=\"radio\" name=\"check_list\" value=\"". $row['SiNo'] . "\" id=\"". $row['SiNo'] . "\"></td>";
echo "<td>" . $row['Name'] . "</td>";
